...
const both = computeBothQueue({ taRows, apps });
const connect = computeConnectQueue({ taRows, apps });
const email = computeEmailQueue({ taRows, apps });
...
console.log('\n── empty input is safe ─────────────────────────────────────────────');
check(computeBothQueue({ taRows: [], apps: [] }).length === 0, 'empty rows → empty both queue');
