 ...
 *
 * WHY THIS EXISTS:
 * The old lock was global single-flight. 7.4 relaxes it so a deep evaluation can
...
reset();
for (const m of ['scan', 'pipeline', 'deep']) check(`${m} may start when idle`, allow(m));

// ── a pipeline chain is running: deep may overlap; scan and a 2nd pipeline may not ──
...
for (const m of ['scan', 'pipeline', 'deep']) check(`${m} is refused while a scan runs`, block(m));
