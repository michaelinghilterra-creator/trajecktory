...
// Row shapes mirror parseTargetTalentMd / parseRecruitersMd: `email` is the clean
...
check(q.length === 2, `queue holds exactly the 2 emailable-at-applied contacts (got ${q.length})`);
check(ids.includes('ta:1'), 'sendable TA contact at an applied company is queued');
check(ids.includes('recruiter:101'), 'risky-but-sendable recruiter at an applied firm is queued');
