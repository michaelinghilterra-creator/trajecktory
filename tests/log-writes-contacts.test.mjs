...
function setupVerify(name, writesOn) {
  const sandbox = makeRepoSandbox(ROOT, name);
  fs.mkdirSync(path.join(sandbox, 'data'));
...
const verifyOff = setupVerify('verify-off', false);
const verifyOn = setupVerify('verify-on', true);
...
check(verifyEvents.length === 1 && verifyEvents[0].payload.ref === 'ta:900001', 'verify-contacts records one person_updated event per changed row');
store.close();
