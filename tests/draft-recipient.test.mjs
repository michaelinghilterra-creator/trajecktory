 ...
 *
 * WHY THIS EXISTS
 * The three contact books number their rows independently, so referral 7088 and
 * target-talent 7088 are two different people. Three handlers in
...
// id 7088 in BOTH books, two different people. This is the collision.
fs.writeFileSync(path.join(sandbox, 'target-talent.md'),
  ...
  '|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|\n' +
  '| 7088 | Umbrella | Stone | Dana | Ms. | Recruiter | Austin | TX | 78701 | 555 | dana@umbrella.example | linkedin.com/in/dana-stone-ex | Not Contacted |  |  |  |\n',
  'utf8');

// Written directly rather than through appendReferralRows, which assigns max+1 and
// would land on 1 in an empty sandbox. The id has to be 7088 on both sides or the
...
fs.writeFileSync(path.join(sandbox, 'referrals.md'),
  ...
  '|---|------|-------------------|--------------------|---------------------|--------|------------|-------|----------|-------|\n' +
  '| 7088 | Priya Raman | 1st-degree LinkedIn connection | Initech |  | Not Asked |  |  | https://www.linkedin.com/in/priya-raman-ex |  |\n',
  'utf8');
const { parseReferralsMd } = await import('../dashboard-web/server/lib/referrals.mjs');
const referral = parseReferralsMd().find(r => r.id === 7088) || {};
...
check(referral.id === 7088, `the referral fixture landed on id 7088 (got ${referral.id})`);

console.log('\n2. A connect note for a referral names the REFERRAL');
{
  const res = await post('/api/linkedin-drafts/connect-note', { source: 'referral', id: 7088 });
...
{
  const res = await post('/api/linkedin-drafts/connect-note', { source: 'ta', id: 7088 });
  const blob = JSON.stringify(res.body);
...
console.log('\n5. Archiving refuses rather than writing to the wrong book');
{
  ...
  const res = await post('/api/linkedin-drafts/archive-contact', { source: 'referral', id: 7088, reason: 'left-company' });
  ...
  const ok = await post('/api/linkedin-drafts/archive-contact', { source: 'ta', id: 7088, reason: 'left-company' });
