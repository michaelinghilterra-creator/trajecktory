...
async function worker() {
  ...
  if (action === 'stale') {
    ...
    process.stdout.write(`${JSON.stringify({ response, snapshot: snapshot(dataDir) })}\n`);
    return;
  }
  ...
  exports.saveConnections([{ first: 'Example', last: 'Persontwo', company: 'Quennox Ratchet Works', url: 'linkedin.com/in/example-person-two' }], 'invented'); steps.push(snapshot(dataDir));
...
check(b.steps[5]['linkedin-connects.json'] === b.steps[4]['linkedin-connects.json'], 'duplicate connection request appends no bytes');
check(!JSON.parse(b.steps[6]['tt-linkedin.json'])['900001'], 'Not Connected deletes the LinkedIn state key');
check(JSON.parse(b.steps[8]['twc-events.json']).length === 1, 'work-search deletion replaces the projected array with the kept event');
...
let scriptRun = spawnSync(process.execPath, ['backfill-linkedin-status.mjs', '--apply'], { cwd: statusBackfill, encoding: 'utf8' });
if (scriptRun.status !== 0) throw new Error(scriptRun.stderr || scriptRun.stdout);
let scriptStore = awaitImportEventStore.openEventStore(path.join(statusBackfill, 'data', 'trajecktory.db'));
