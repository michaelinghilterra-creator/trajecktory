...
const tagged = parseVerifyTag('l.reyes@brightwave.example [v:ok:hunter:2026-07-22:96]');
...
check(tagged.address === 'l.reyes@brightwave.example', 'address stripped clean of the tag');

const noScore = parseVerifyTag('x@y.example [v:bounced:notes:2026-06-10]');
...
const legacy = parseVerifyTag('j.kim@brightwave.example [bounced 2026-06-11: a@b.example] [v:invalid:notes:2026-06-11]');
...
const bogus = parseVerifyTag('x@y.example [v:wat:manual:2026-01-01]');
...
const rt = parseVerifyTag(`a@b.example ${formatVerifyTag({ state: 'risky', source: 'mv', date: '2026-07-23' })}`);
check(rt.state === 'risky' && rt.source === 'mv' && rt.date === '2026-07-23', 'format→parse round-trips');

// ── setVerifyTag ─────────────────────────────────────────────────────────────
const set1 = setVerifyTag('a@b.example', { state: 'ok', source: 'hunter', date: '2026-07-22' });
check(set1 === 'a@b.example [v:ok:hunter:2026-07-22]', 'set adds a tag');
const set2 = setVerifyTag('a@b.example [v:unverified]', { state: 'bounced', source: 'gmail', date: '2026-07-25' });
check(set2 === 'a@b.example [v:bounced:gmail:2026-07-25]', 'set REPLACES an existing v tag');
const set3 = setVerifyTag('a@b.example [bounced 2026-06-11: x@y.example]', { state: 'invalid', source: 'notes', date: '2026-06-11' });
...
check(isSendable({ email: 'a@b.example', verified: { state: 'ok' } }), 'row: address + ok → sendable');
check(!isSendable({ email: '', verified: { state: 'ok' } }), 'row: no address → not sendable even if ok');
check(!isSendable({ email: 'a@b.example', verified: { state: 'bounced' } }), 'row: bounced → not sendable');
check(!isSendable({ email: 'a@b.example' }), 'row: missing verified → treated as unverified → not sendable');
