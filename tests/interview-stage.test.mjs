...
check(stageOf('VP of Example Ratchet Assembly').stage === 'Interview', 'a VP is an Interview');
...
check(stageOf('Example Widget Assembler').stage === null && stageOf('Example Widget Assembler').needs_owner === true, 'a title with no keyword asks the owner');
check(['Senior Recruiter', 'VP of Example Ratchet Assembly', 'Panel interview'].every((t) => stageOf(t).needs_owner === false), 'a proposed stage never asks the owner');
