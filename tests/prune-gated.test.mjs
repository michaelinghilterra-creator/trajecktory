...

{
  ...
  const undatedRow = `- [!] ${undatedUrl} | Example | Undated`;
  const badRow = `- [!] ${badUrl} | Example | Bad date`;
  const r = runCase({
    text: pipeline([
      undatedRow,
      badRow,
  ...
  const after = r.after.toString('utf8');
  // Assert on the whole row, matching the other cases. A bare .includes() on a
  // URL reads as an incomplete URL-trust check to CodeQL, and the row is the
  // more precise assertion anyway.
  check(r.code === 0 && !after.includes(undatedRow) && !after.includes(badRow),
