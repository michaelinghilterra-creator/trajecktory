...
3. **Auto-push cheat sheet to Obsidian** (without asking):
   ...
   - Read `modes/cheat-sheet.md` and generate the cheat sheet for this company + role
   - Resolve filename tokens: `{applied_date_mdy}` = today's date as `MM-DD-YYYY` (since we just marked it Applied), `{company}` = company name, `{role}` = role title. Strip Windows-forbidden chars (`\ / : * ? " < > |` → `-`).
   - Push to `{applied_folder}/{resolved_filename}.md` using the Obsidian MCP:
     - Check if a note for this company/role already exists via `mcp__obsidian-vault__obsidian_simple_search` (search by company + role, not filename — older notes may use a different format)
     - If exists at same path: `mcp__obsidian-vault__obsidian_patch_content` with `operation: replace`
