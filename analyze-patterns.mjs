...
// Status normalization and outcome classification live in lib/outcome.mjs
// (imported above) so they are testable and shared.

// --- Parse applications.md (canonical parser in lib/tracker.mjs) ---
function parseTracker() {
  if (!existsSync(APPS_FILE)) return [];
  return parseTrackerMd(readFileSync(APPS_FILE, 'utf-8'));
}
...
function analyze() {
  ...
  const scoresByOutcome = Object.fromEntries(OUTCOMES.map(o => [o, []]));
  for (const e of enriched) {
  ...
  const scoreComparison = Object.fromEntries(OUTCOMES.map(o => [o, scoreStats(scoresByOutcome[o])]));

  // --- Archetype breakdown ---
  const archetypeMap = new Map();
  for (const e of enriched) {
    const arch = e.report?.archetype || 'Unknown';
    if (!archetypeMap.has(arch)) archetypeMap.set(arch, zeroOutcomeCounts());
  ...
  for (const e of enriched) {
    ...
    if (!remoteMap.has(policy)) remoteMap.set(policy, zeroOutcomeCounts());
  ...
  for (const e of enriched) {
    ...
    if (!sizeMap.has(size)) sizeMap.set(size, zeroOutcomeCounts());
  ...
  if (bestArchetype && bestArchetype.conversionRate > 0) {
    recommendations.push({
      ...
      reasoning: `${bestArchetype.positive} of ${bestArchetype.positive + bestArchetype.negative} decided applications in this archetype led to positive outcomes.`,
      impact: 'medium',
    });
  }
...
function printSummary(result) {
  ...
  const funnelOrder = ['evaluated', 'applied', 'responded', 'phone screen', '1st interview', '2nd interview', '3rd interview', '4th interview', 'offer', 'rejected', 'discarded', 'skip'];
