...
System-layer template files used by trajecktory scripts and modes. These files are auto-updated when you run `npm run update` -- put user customizations in the user-layer files instead (see DATA_CONTRACT.md).
...
**To activate:** Copy to project root as `portals.yml` and customize the `title_filter.matrix` blocks (functions_bare, functions_ranked, seniority) for your target roles. Add or remove companies as needed.
