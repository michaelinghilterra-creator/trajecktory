...
# INTEGRITY. Everything downloaded here is compiled into an installer handed to
# other people, so a bad fetch on this machine becomes a bad binary on theirs.
# HTTPS proves who served the file, not that the file is the one the project
# published, and it does nothing about a compromised mirror or a poisoned cache.
#
# Node is checked against the vendor's OWN manifest from the same release
# directory rather than a hash pinned here. A pinned constant goes stale on every
# version bump and gets "fixed" by pasting whatever the new download hashed to,
# which is not a check at all. SHASUMS256.txt is regenerated by the release, so
# bumping $NodeVersion needs no edit here and the check keeps meaning something.
...
Write-Host "Verifying Node against $NodeSumsUrl ..."
$sums = (Invoke-WebRequest -Uri $NodeSumsUrl -UseBasicParsing).Content
$wantLine = ($sums -split "`n") | Where-Object { $_ -match [regex]::Escape("$NodeZipName.zip") } | Select-Object -First 1
if (-not $wantLine) { throw "No SHASUMS256 entry for $NodeZipName.zip at $NodeSumsUrl" }
$WantNodeHash = ($wantLine -split '\s+')[0].Trim().ToUpperInvariant()
$GotNodeHash  = (Get-FileHash $zip -Algorithm SHA256).Hash.ToUpperInvariant()
if ($GotNodeHash -ne $WantNodeHash) {
  throw "Node archive FAILED integrity check.`n  expected $WantNodeHash`n  got      $GotNodeHash`n  from     $NodeUrl`nRefusing to build an installer around an unverified binary."
}
Write-Host "  Node SHA256 OK"
...
# INTEGRITY + AUTHENTICITY, which the version check above is NOT. ProductVersion
# is metadata inside the file: a tampered installer carries whatever version
# string its author chose, so reading it back proves only that the right release
# was requested. This binary is run with /VERYSILENT on every user's machine, so
# it needs a stronger answer than "the download succeeded".
#
# Two independent checks, because they fail differently. The signature proves who
# built it and that it has not been altered since, and keeps working across
# version bumps. The pinned hash proves it is the exact artifact this build was
# developed against, and catches a legitimately-signed but DIFFERENT build
# slipping in. Git for Windows publishes no SHASUMS manifest, hence a constant
# here rather than the vendor-manifest approach used for Node above.
#
# WHEN YOU BUMP $GitVersion / $GitRevision this hash must change too. Do not copy
# whatever the new download produced without checking the signature first: a hash
# updated to match an unverified file is a rubber stamp, not a check.
$GitExpectedSha = 'AF12577D0FDFF74243A5988197AA49B957D5044EDC17004F6DDF0768996F1DCA'  # 2.55.0.3
$GitExpectedSigner = 'CN=Johannes Schindelin'

$sig = Get-AuthenticodeSignature $GitOut
if ($sig.Status -ne 'Valid') {
  throw "Git installer signature is '$($sig.Status)', not Valid. Refusing to bundle it. Source: $GitUrl"
}
if ($sig.SignerCertificate.Subject -notlike "$GitExpectedSigner*") {
  throw "Git installer is signed by an unexpected party: '$($sig.SignerCertificate.Subject)'. Expected $GitExpectedSigner. Source: $GitUrl"
}
$GitGotSha = (Get-FileHash $GitOut -Algorithm SHA256).Hash.ToUpperInvariant()
if ($GitGotSha -ne $GitExpectedSha.ToUpperInvariant()) {
  throw "Git installer FAILED its pinned hash.`n  expected $GitExpectedSha`n  got      $GitGotSha`n  from     $GitUrl`nIf you bumped the Git version, verify the new file's signature and then update `$GitExpectedSha."
}
Write-Host "  Git installer signature + SHA256 OK ($($sig.SignerCertificate.Subject.Split(',')[0]))"
