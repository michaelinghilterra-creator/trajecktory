...
- Setup (other)
  ...
  - Day-to-day guide
    ...
    - 03 Today
    - 04 Finding roles: Scan & Evaluate
    - 05 Pipeline
    - 06 Reading a report
    - 07 Follow-Ups
    - 08 Network
...
| Pipeline: Roles, Active | Shows non-terminal evaluated roles with status, score, source, date, and filters. | Keeps current opportunities in a focused working ledger. | RevOps | Filter or open an active role. | `/api/applications` | `data/applications.md`, reports | Yes |
...
| Pipeline: Discovery, Pending | Lists unchecked roles waiting for evaluation. | Makes the incoming queue visible before AI spend. | RevOps | Open the source. | `/api/pipeline/inbox` | `data/pipeline.md`, `data/scan-history.tsv` | Yes |
...
| Guide 04: Finding roles: Scan & Evaluate | Explains API Scan, Agent Scan, JD resolution, liveness gating, and evaluation. | Clarifies how roles enter the system and where AI is used. | Shared | Read the acquisition workflow. | None | derived/live from `dashboard-web/src/guide.jsx` | No |
...
| `resolve-jds.mjs` | Resolves supported SPA job pages through ATS APIs and replaces queue URLs with local snapshots. | `data/pipeline.md`, `portals.yml`, `data/resolve-fail-counts.json`, ATS APIs | `jds/*.md`, updated `data/pipeline.md`, fail counts, `data/gate-history.tsv` | Run before evaluation so agents can read the full JD. |
...
| `reconcile-triage.mjs` | Marks queue rows recorded in the Spark pre-filter discard log, or already fully evaluated, as done. | `data/pipeline.md`, `data/triage-results.tsv`, `data/applications.md` | Updates `data/pipeline.md` only with `--apply` | Run after the optional Spark pre-filter or when pending counts look stale. |
| `spark-prefilter.mjs` | Scores pending postings on your own local OpenAI-compatible endpoint and discards the ones below the threshold, so no Claude evaluation is spent on them. Inert unless `TJK_SPARK_URL` is set. Never discards on a failure: an unreachable endpoint, an unparseable output or an unreadable snapshot all leave the row pending. | `data/pipeline.md`, `jds/*.md` snapshots, `modes/oferta.md`, `config/profile.yml`, the configured endpoint | Appends discard rows to `data/triage-results.tsv` and `prefiltered` rows to `data/gate-history.tsv`; writes raw model output to `data/spark-prefilter/<date>/` — all only with `--apply` | Run after `reconcile-triage.mjs` and before the batch; follow it with a second `reconcile-triage.mjs --apply`. |
| `data-audit.mjs` | Read-only integrity audit of the user layer: unexpected writes in a time window, tracker id and report-link invariants, orphaned reports, sidecar ids with no row, queue rows pointing at missing snapshots, and whether a readable restore point exists. Repairs nothing; prints findings for a human, because several of them have more than one right answer. | `data/applications.md`, `reports/*`, `jds/*`, `data/pipeline.md`, sidecar JSON/TSV | Nothing. It never writes. | Run after a batch, an experiment, or any stretch of one-off scripts touching `data/`. |
