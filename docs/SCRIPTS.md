...
| `npm run pdf` | `generate-pdf.mjs` | Convert HTML to ATS-optimized PDF (legacy CV flow) |
| `npm run sync-check` | `cv-sync-check.mjs` | Validate CV/profile consistency |
| `npm run lint` | `eslint .` | Lint the Node code (.mjs engine, server, libs, tests) |
...
directly with `node generate-docx-from-template.mjs`). It is not an `npm run`
script. The `pdf` flow above is the legacy HTML-to-PDF path.
...
The default CV path is the docx generator (see the note under Quick Reference).
This HTML-to-PDF flow is kept for the legacy `pdf` mode.
...
Validates that the trajecktory setup is internally consistent: `cv.md` exists and is not too short, `config/profile.yml` exists with required fields, no hardcoded metrics in `modes/_shared.md` or `batch/batch-prompt.md`, and `article-digest.md` freshness (warns if older than 30 days).
...
Checks whether a newer version of trajecktory is available upstream. Outputs JSON to stdout:
