...
<p align="center">
  <a href="https://x.com/santifer"><img src="docs/hero-banner.jpg" alt="trajecktory -- 멀티 에이전트 취업 시스템" width="800"></a>
</p>
...
<p align="center">
  <img src="docs/demo.gif" alt="trajecktory 데모" width="800">
</p>
...
trajecktory는 AI 코딩 CLI를 취업 활동 전체를 관리하는 커맨드 센터로 바꿔줍니다. 스프레드시트에서 수동으로 지원 현황을 관리하는 대신, AI 파이프라인이 알아서 처리합니다:
...
> **중요: 이 도구는 무차별 지원 도구가 아닙니다.** trajecktory는 필터입니다 -- 수백 개의 공고 중 당신의 시간을 투자할 가치가 있는 소수의 공고를 찾아줍니다. 4.0/5 미만의 공고에는 지원하지 않는 것을 강력히 권장합니다. 당신의 시간도, 채용 담당자의 시간도 소중합니다. 제출 전에 항상 직접 검토하세요.

trajecktory는 에이전트 기반으로 작동합니다: Claude Code가 Playwright로 채용 페이지를 탐색하고, 키워드 매칭이 아닌 이력서와 JD를 비교 분석하여 적합도를 판단하고, 공고별로 이력서를 맞춤 생성합니다.
...
trajecktory는 다양한 모드를 가진 하나의 슬래시 커맨드입니다:
...
채용 공고 URL이나 설명을 바로 붙여넣어도 됩니다 -- trajecktory가 자동으로 감지하여 전체 파이프라인을 실행합니다.
...
└──────────┬─────────────┘
           │
┌──────────▼─────────────┐
│  A-G 평가               │  이력서 기반 매칭도 및 갭 분석, 연봉 리서치, STAR 스토리
│  (cv.md 참조)           │
└──────────┬─────────────┘
           │
      ┌────┼────┐
...
웹 대시보드로 파이프라인을 시각적으로 탐색할 수 있습니다:
...
├── dashboard-web/               # 웹 대시보드 (Express + React)
...
Santiago입니다 -- Head of Applied AI, 전직 창업자 (직접 사업을 만들고 매각했으며, 아직도 제 이름으로 운영되고 있습니다). 제 취업 활동을 관리하기 위해 trajecktory를 만들었습니다. 효과가 있었습니다: 이 시스템으로 현재 포지션에 합격했습니다.
...
**trajecktory는 로컬 오픈소스 도구이며, 별도의 호스팅 서비스가 아닙니다.** 이 소프트웨어를 사용함으로써 다음 사항에 동의하는 것으로 간주됩니다:
