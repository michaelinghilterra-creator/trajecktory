...
window.OverviewTab = function OverviewTab({ apps, onOpen, onAction, setTab, search }) {
  ...
  // Activity insights (28-day window)
  const activityInsights = useMemoO(() => {
    ...
    const last28 = apps.filter(a => window.daysAgo(a.date) <= 27).length;
    const avgPerWeek = (last28 / 4).toFixed(1);
    const trend = last7 - prior7;
    // Peak day in window
    const dayCounts = {};
    apps.forEach(a => { if (window.daysAgo(a.date) <= 27) dayCounts[a.date] = (dayCounts[a.date] || 0) + 1; });
