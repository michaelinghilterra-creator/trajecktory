...
window.OverviewTab = function OverviewTab({ apps, onOpen, onAction, setTab, search }) {
  ...
  // Activity insights (60-day window)
  const activityInsights = useMemoO(() => {
    ...
    const last60 = apps.filter(a => window.daysAgo(a.date) <= 59).length;
    const avgPerWeek = (last60 * 7 / 60).toFixed(1);
    const trend = last7 - prior7;
    // Peak day in window
    const dayCounts = {};
    apps.forEach(a => { if (window.daysAgo(a.date) <= 59) dayCounts[a.date] = (dayCounts[a.date] || 0) + 1; });
...
const toggleRow = (id) => setSelected(s => {
  ...
  return (
    <div className="col" style={{ gap: 16 }}>
      ...
      {/* Activity · last 60 days — full-width band on top */}
      <div className="card padded-lg" style={{ display: "flex", flexDirection: "column" }}>
        <div className="card-head">
          <span className="card-title">Activity · last 60 days</span>
          <span className="card-meta mono">
            {apps.filter(a => window.daysAgo(a.date) <= 60).length} entries &nbsp;·&nbsp;
        ...
        <window.Timeline apps={apps} days={60} height={72} />
