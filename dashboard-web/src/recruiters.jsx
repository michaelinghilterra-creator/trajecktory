  ...
  return (
    <div className="ai-compose">
      ...
      <div className="ai-tone">
        ...
        {hasReceived && (
          <button className="btn sm" onClick={() => gen('reply')} style={{ marginLeft: 'auto' }}
            title="Draft a reply to this recruiter's most recent email, using the thread for context">
...
// A recruiter now opens the SAME shared card as a TA or referral contact:
// window.ContactPanel driven by the recruiter adapter (CONTACT_CFG_RECRUITER in
// target-talent.jsx). It fetches its own detail by id, so this is a thin shell.
window.RecruiterDrawer = function RecruiterDrawer({ id, onClose, onUpdate }) {
  const open = id != null;
  const Shared = window.ContactPanel;
  const cfg = window.CONTACT_CFG_RECRUITER;
  return (
    <>
      ...
      <div className={"drawer wide" + (open ? " open" : "")} style={{ transform: open ? "translateX(0)" : "translateX(100%)" }}>
        {open && Shared && cfg && <Shared id={id} cfg={cfg} onClose={onClose} onUpdate={onUpdate} />}
      </div>
    </>
  );
};
