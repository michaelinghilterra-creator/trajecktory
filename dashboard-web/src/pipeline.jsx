...
function PipelineDrawer({ app, onClose, onAction, onStatusChange, isStale = () => false, onFollowupChange = () => {} }) {
  ...
  if (st === 'Evaluated') {
    ...
    // Re-queue a near-threshold auto-discard in the 3.0 to 3.5 band.
    ...
    if (window.oldStatus(app) === 'Discarded' && typeof app.score === 'number' && app.score >= 3.0 && app.score < 3.5) {
        ...
        <div className="tbl-wrap" style={{ border: 'none', borderRadius: 0, background: 'transparent' }}>
          <table className="tbl pl-tbl">
            <thead><tr>{cols.map(col => (
              <th key={col.k} style={{ width: col.w, textAlign: 'left' }} className={sortKey === col.k ? 'sorted' : ''} role="button" tabIndex={0}
...
window.PipelineTab = function PipelineTab({ apps, view, setView, filters, setFilters, onOpen, onQuickAction, onDataChanged, search, compTweaks }) {
  ...
  // found-but-unevaluated or gated role is never invisible. Refresh on mount,
  // focus, and after any scan/evaluation that changes `apps`.
  ...
  return (
    <div className="col" style={{ gap: 0 }}>
      ...
      {subView === 'table' && (
        <TableView apps={apps} filtered={filtered} filters={filters} setFilters={setFilters} search={search} setSearch={() => {}} onOpen={handleOpen} selId={selId} onExport={onExport} isStale={isStale} staleDays={staleDays} />
      )}
      {subView === 'discovery' && (
