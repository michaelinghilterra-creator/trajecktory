...
function dashboardConstraints(mode, opts) {
  ...
  if (mode === 'pipeline') {
    ...
    return ' ' + common +
      ...
      ' Do not run gate-pipeline.mjs or any browser tool; just evaluate the pending unchecked URLs as they are. Read each job description with WebFetch. If WebFetch returns only a JavaScript shell or an empty body (common on Workday and Ashby single-page apps), do NOT reconstruct the JD from WebSearch or aggregator mirrors — a stitched-together JD produces a confident score for a posting you never actually read, and that has shipped CLOSED roles as high scores. Instead, append one tab-separated line (url, company, role) to data/needs-manual-jd.tsv (create it with that exact header row if it is missing), mark that URL done in data/pipeline.md ([ ] to [x]), and write NO report and NO tracker TSV for it. The user will confirm the posting is live and paste the JD text themselves.' +
