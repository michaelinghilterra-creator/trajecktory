...
trajecktory превращает любой AI-CLI в полноценный командный центр для поиска работы. Вместо ручного ведения таблиц с откликами вы получаете ИИ-пайплайн, который:
...
> **Важно: это НЕ инструмент для массовой рассылки.** trajecktory — это фильтр. Он помогает найти те немногие вакансии, которые стоят вашего времени, из сотен. Система настоятельно рекомендует не откликаться на вакансии с оценкой ниже 4.0/5. Ваше время ценно, как и время рекрутера. Всегда проверяйте перед отправкой.
...
trajecktory — одна slash-команда с множеством режимов:
...
Или просто вставь URL вакансии — trajecktory автоматически определит его и запустит полный пайплайн.
...
└────────┬─────────┘
         │
┌────────┼─────────┐
│  Оценка A–G      │  Совпадение, пробелы, исследование компенсации, STAR-истории
│  (читает cv.md)  │
└────────┬─────────┘
         │
    ┌────┼─────┐
...
trajecktory поддерживает ключевые российские джоб-борды:
...
Веб-дашборд для визуального просмотра пайплайна:
...
├── dashboard-web/               # Веб-панель (Express + React)
...
**trajecktory — локальный open-source инструмент, НЕ хостинг-сервис.** Используя это ПО, вы подтверждаете:
