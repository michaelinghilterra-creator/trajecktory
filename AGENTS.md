...
Copy `templates/portals.example.yml` → `portals.yml`. If they gave target roles in Step 2, update `title_filter.matrix.functions_bare`, `functions_ranked`, and `seniority` to match.
