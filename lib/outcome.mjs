...
const INTERVIEW_STAGES = ['phone screen', '1st interview', '2nd interview', '3rd interview', 'interview'];

export function classifyOutcome(status) {
  const s = normalizeStatus(status);
  // 'positive' is a WIN: only an Offer counts. Applied / interviewing
  // are in-flight, not decided, so they fall through to 'pending' — otherwise the
  // conversionRate ("win rate among applications that reached a decision") counted
  // still-open applications as wins and overstated the rate.
  if (s === 'offer') return 'positive';
