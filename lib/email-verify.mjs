...
export function parseVerifyTag(emailCell) {
  ...
  // Strip every bracket tag, then normalize whitespace. Two ReDoS guards, since this
  // address can come from a request body: (1) no surrounding \s* — `\s*[...]\s*` under a
  // global replace is quadratic on long whitespace; (2) the class excludes `[` as well as
  // `]` (`[^\][]*`), so an unclosed `[v:[v:[v:…` cannot make the inner star rescan to the
  // end from every start. A real tag body never contains `[`, so matching is unchanged.
  const address = raw.replace(/\[[^\][]*\]/g, '').replace(/\s{2,}/g, ' ').trim();
...
export function setVerifyTag(emailCell, verify) {
  const raw = String(emailCell ?? '');
  // ReDoS-safe tag strip: no surrounding \s* (quadratic on whitespace) and a class that
  // excludes `[` as well as `]` (linear on unclosed `[v:[v:…`). See parseVerifyTag.
  const withoutV = raw.replace(/\[v:[^\][]*\]/gi, ' ').replace(/\s{2,}/g, ' ').trim();
  if (!withoutV.replace(/\[[^\][]*\]/g, '').trim()) return withoutV; // no address
