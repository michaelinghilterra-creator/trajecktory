...
// guarantees the output parses back as exactly 10 cells. An empty Resume cell is
// written as the '—' placeholder the file uses, which parseTrackerLine reads
// back as null, so parse(format(row)) round-trips.
export function formatTrackerLine(fields) {
  const cells = TRACKER_COLUMNS.map((key) => {
    const cell = sanitizeTrackerCell(fields[key]);
    if (key === 'resume' && !cell) return '—';
    return cell;
  });
  return `| ${cells.join(' | ')} |`;
}
