...
export function parseTrackerLine(line) {
  ...
  return {
    ...
    urlCell,           // raw 11th cell, so callers can tell "no URL" from a stray pipe
...
// Placeholders that mean "this row simply has no posting URL".
const URL_CELL_BLANK = /^(|-|—|–|n\/a|none)$/i;

// Does this row's 11th cell indicate a STRAY PIPE, as opposed to no URL at all?
//
// Both look identical through `url`, which is null either way, and treating the
// two the same made the tracker parser warn "notes may be truncated" on 62 rows
// of a real file where not one byte was lost. 58 of them held the em dash that
// formatTrackerLine ITSELF writes for an empty url cell, and 4 held a `local:`
// reference to a self-sourced JD on disk, which lib/identity.mjs canonicalizes
// deliberately. A warning that fires on healthy data is worse than no warning,
// because it teaches you to scroll past the real one.
export function hasStrayPipe(row) {
  if (!row) return false;
  if (row.cellCount > 11) return true;            // more cells than the schema has
  if (row.cellCount !== 11 || row.url) return false;
  const cell = String(row.urlCell || '').trim();
  if (URL_CELL_BLANK.test(cell)) return false;    // no URL on this row, which is normal
  if (/^local:/i.test(cell)) return false;        // self-sourced file reference
  return true;                                    // something else landed in the URL slot
}
