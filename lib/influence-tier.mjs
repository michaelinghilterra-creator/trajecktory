...
const SENIORITY_PHRASES = Object.freeze([
  Object.freeze([
    'chief', 'ceo', 'cro', 'coo', 'cfo', 'cto', 'cmo', 'chro', 'president',
...
const FUNCTION_PHRASES = Object.freeze({
  ...
  talent: Object.freeze([
    ...
    'people operations', 'hr', 'human resources', 'chro', 'talent partner',
...
function classifyTitle(title, { track } = {}) {
  ...
  if (group === 'talent') {
    const isPeopleOrHr = [
      'people', 'people operations', 'hr', 'human resources', 'chro',
    ].some(phrase => containsWholeRun(paddedTitle, phrase));
    const isRecruiting = [
      'talent acquisition', 'recruiting', 'recruiter', 'recruitment', 'talent partner',
    ].some(phrase => containsWholeRun(paddedTitle, phrase));
    const isPeopleLeader = level === 3
      || containsWholeRun(paddedTitle, 'vp')
      || containsWholeRun(paddedTitle, 'head');
    if (isPeopleOrHr && !isRecruiting && isPeopleLeader) return 'exec';
    return 'ta';
  }
...
const TIER_TAG_RE = /\[tier:([a-z]{1,10})\]/i;
const ALL_TIER_TAGS_RE = /\[tier:[a-z]{1,10}\]/gi;
...
function hasExplicitTier(notes) {
  return typeof notes === 'string'
    && (TIER_TAG_RE.test(notes) || /\[principal\]/i.test(notes));
}

function resolveInfluenceTier({ notes, title, track } = {}) {
  if (hasExplicitTier(notes)) {
    return { tier: parseInfluenceTier(notes), source: 'tag' };
  }

  const titleTier = classifyTitle(title, { track });
  if (titleTier !== null) return { tier: titleTier, source: 'title' };

  // A null title classification means the title tells us nothing. It is not an
  // assessment that the person is a gatekeeper. Ranking needs the same default
  // tier in both cases, while source preserves this important asymmetry.
