...
// These are the query keys that DISTINGUISH one posting from another and so must
// survive canonicalization. Erring toward keeping a key is the safe direction: a
// non-id key left in only risks treating one posting as two (a visible duplicate
// row), whereas STRIPPING a real id key collapses two different reqs into one and
// silently suppresses a live posting — the failure the whole module guards against.
// So this list is broad; add a key whenever an ATS is found to carry its posting id
// in a query param not already here.
const ID_QUERY_KEYS = new Set([
  'gh_jid',
  'jobid', 'job_id', 'job', 'jobcode', 'job_code',
  'reqid', 'req_id', 'req', 'jobreqid', 'jobreq_id',
  'postingid', 'posting_id', 'posting',
  'positionid', 'position_id', 'position',
  'openingid', 'opening_id', 'opening', 'jobopeningid',
  'vacancyid', 'vacancy_id', 'vacancy',
  'ccid', 'cid', 'pid', 'rid', 'jrid',
]);
...
export function canonicalUrl(url) {
  ...
  // A `local:` reference is a self-sourced FILE PATH, not a web posting. It is a
  // usable identity only if it points at one real file. It is NOT usable when the
  // path still carries an unexpanded template/shell variable, because the
  // substitution never ran and the SAME literal then stands in for many different
  // files. On 2026-08-10 a batch wrote a literal `local:...\...$file` — `$file`
  // never expanded — as the url of SIX unrelated reports at six different
  // employers. All six canonicalized to one key, and Tier-0 URL matching
  // (deliberately company-blind — "same url = same posting") then merged one
  // employer's re-eval onto another employer's row and dropped four more
  // evaluations as "duplicates". Returning '' here marks the reference UNRESOLVABLE,
  // so identity falls back to the company-gated signals and unrelated postings can
  // never collapse onto one another again. Scoped to `local:` so http percent-
  // encoding (%2F, %20, …) is never mistaken for an unexpanded %VAR%.
  if (/^local:/i.test(url)) {
    const body = url.slice(url.indexOf(':') + 1);
    // $file, ${file}, %VAR%, {{file}} — any unexpanded interpolation token. Every
    // branch is linear: the mustache branch matches only the opening `{{` (its
    // presence is enough to flag), NOT `{{.*?}}`, whose lazy inner match backtracks
    // on input like `{{{{…` and is a polynomial-ReDoS sink — and `body` comes from
    // scanned, i.e. untrusted, URLs (CodeQL js/polynomial-redos).
    if (!body.trim() || /\$\{?\w+\}?|%\w+%|\{\{/.test(body)) return '';
  }
...
export function sameRole(a, b) {
  return sameRoleSignature(roleSignature(a), roleSignature(b));
}

// sameRole on precomputed signatures, for callers comparing one title against many.
export function sameRoleSignature(sigA, sigB) {
...
export function buildDecidedIndex({ appsPath, rootDir }) {
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    if (!key) {
      // The row HAS a url cell, but it is not a trustworthy identity (a junk local
      // path — see canonicalUrl). Treat it exactly like a row with no url at all:
      // fall back to company+role rather than letting it vanish from every index,
      // which would make it invisible to dedup.
      const co = normalizeCompany(row.company);
      if (!noUrlByCompany.has(co)) noUrlByCompany.set(co, []);
      noUrlByCompany.get(co).push({ num: row.num, status: row.status, company: row.company, role: row.role });
      continue;
...
// ── Decided-role index (TRIAGE-QUEUE DISPLAY ONLY) ────────────────────────────
// Company+role → every decided tracker row (ANY status, url or not). This is a
// DELIBERATELY broader net than buildActiveRoleIndex, and it exists for exactly
// one caller: the triage-results endpoint, which renders the "what should I look
// at next" ranked queue.
//
// WHY IT IS SAFE HERE BUT NOT IN findDecided:
// findDecided is the EVALUATION/dedup authority, and it refuses to collapse two
// different trustworthy URLs onto one identity — because merging two genuinely
// distinct requisitions onto a single tracker row loses a real job, the failure
// this whole module was written to stop. The triage queue is a different context:
// a card there is advisory, not a tracker row, and the endpoint SUPPRESSES a card
// (moves it to the visible "already handled" list, tagged with the row it
// matched) rather than deleting it. So the worst case here is a same-title
// different-req card shown under "already handled" instead of "new" — a card the
// user can still see and click through — not a lost posting.
//
// THE BUG THIS FIXES: a company posts one role under two requisition URLs (a
// common shape when the same opening is listed once per hub office, each with its
// own ATS id). Deep-diving one URL records a tracker row; the SIBLING url never
// canonical-matches it, so findDecided leaves it as an active triage card forever,
// showing its stale pre-filter score (a "phantom" the user has to notice and
// dismiss by hand every time). Matching the sibling by company+role moves it to
// suppressed automatically. Returns the HIGHEST-numbered matching row (most
// recent decision).
export function buildDecidedRoleIndex({ appsPath, rootDir }) {
  const byCompany = new Map();
  let text = '';
  try { text = readFileSync(appsPath, 'utf-8'); } catch { return { byCompany }; }
  for (const line of text.split(/\r?\n/)) {
    ...
    const co = normalizeCompany(row.company);
    if (!byCompany.has(co)) byCompany.set(co, []);
    byCompany.get(co).push({ num: row.num, status: row.status, company: row.company, role: row.role });
  }
  return { byCompany };
}

// Same company + same role (sameRole) as an already-decided tracker row? Returns
// the most-recent (highest-num) match, or null. company+role is a HINT, never
// proof (see sameRole's own caveat), so a caller may only use this to move a card
// to the auditable "already handled" list — never to delete a row or merge reqs.
export function findDecidedRole(roleIndex, hint) {
  if (!roleIndex || !hint || !hint.company || !hint.role) return null;
  const candidates = roleIndex.byCompany.get(normalizeCompany(hint.company)) || [];
  let best = null;
  for (const c of candidates) {
    if (!sameRole(c.role, hint.role)) continue;
    if (!best || c.num > best.num) best = c; // highest row num = most recent decision
  }
  return best;
}
