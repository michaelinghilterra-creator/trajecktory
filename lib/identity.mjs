...
// Resolve a local JD snapshot back to the posting URL recorded in its header.
// Old snapshot names remain valid because identity comes from file content, not
// from the filename. A missing or malformed snapshot is unresolved and callers
// must fail open.
export function sourceUrlFromSnapshot(localRef, rootDir, readFile = readFileSync) {
  if (!rootDir || typeof localRef !== 'string') return null;
  const rel = localRef.replace(/^local:/i, '').trim();
  if (!rel) return null;

  const snapshotsRoot = resolve(rootDir, 'jds');
  const full = resolve(rootDir, rel);
  if (full !== snapshotsRoot && !full.startsWith(snapshotsRoot + sep)) return null;

  let body;
  try {
    body = readFile(full, 'utf8');
  } catch {
    return null;
  }

  const labelled = body.match(/^\*{0,2}(?:Source )?URL:\*{0,2}\s*(https?:\/\/\S+)/im);
  // No bare-URL fallback: the first link in a JD body is often the employer's
  // site, and a wrong identity could hide a live posting as already decided.
  return labelled ? labelled[1].trim() : null;
}
// Canonical identity for either a posting URL or a local:jds snapshot reference.
export function canonicalPostingUrl(value, rootDir) {
  const input = String(value || '');
  if (!/^local:/i.test(input)) return canonicalUrl(input);
  const sourceUrl = sourceUrlFromSnapshot(input, rootDir);
  return canonicalUrl(sourceUrl || input);
}
...
export function buildDecidedIndex({ appsPath, rootDir }) {
  ...
  let text = '';
  try { text = readFileSync(appsPath, 'utf-8'); } catch { return { byUrl, ambiguous, noUrlByCompany, rootDir }; }
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    const key = canonicalPostingUrl(raw, rootDir);
    if (!key) {
  ...
  return { byUrl, ambiguous, noUrlByCompany, rootDir };
}
...
export function findDecided(index, url, hint) {
  ...
  if (url) {
    const key = canonicalPostingUrl(url, index.rootDir);
