...
const JSON_EFFECT_OPS = new Set(['json_replace', 'json_set', 'json_delete', 'json_append', 'json_nested_append']);
...
export function validateLegacyEffects(effects) {
  ...
  effects.forEach((effect, index) => {
    ...
    if (JSON_EFFECT_OPS.has(effect.op)) {
      ...
      } else if (effect.op === 'json_nested_append') {
        assertOnlyKeys(effect, new Set(['file', 'op', 'key', 'item']), label);
        if (!Object.hasOwn(effect, 'item')) throw new Error(`${label}.item: is required`);
        assertJsonValue(effect.item, `${label}.item`);
      } else {
      ...
      if ((effect.op === 'json_set' || effect.op === 'json_delete' || effect.op === 'json_nested_append')
...
function fileEvents(store, file) {
...
export function activeLegacyFileEvents(store, file) {
  if (!isKnownFile(file)) throw new Error(`unknown legacy file: ${file}`);
  const voided = voidedEventIds(store);
  return fileEvents(store, file)
...
function applyJsonEffect(state, file, effect) {
  ...
  if (effect.op === 'json_append') {
    ...
    if (!state.document || typeof state.document !== 'object' || Array.isArray(state.document)) {
      throw new Error(`json_nested_append requires an object document: ${file}`);
    }
    if (!Object.hasOwn(state.document, effect.key)) state.document[effect.key] = [];
    if (!Array.isArray(state.document[effect.key])) {
      throw new Error(`json_nested_append requires an array at document[${effect.key}]: ${file}`);
    }
    state.document[effect.key].push(effect.item);
  } else {
    if (!state.document || typeof state.document !== 'object' || Array.isArray(state.document)) {
