...
const EFFECT_OPS = new Set(['row_upsert', 'row_delete', 'file_replace']);
...
function isKnownFile(file) {
  return TABLE_FILE_SET.has(file) || correspondenceParts(file) !== null;
}
...
function makeTableProjection(store, file) {
  ...
  for (const event of events) {
    ...
    if (Number.isInteger(payload.line_index) && typeof payload.raw === 'string') {
      rawByRowId.set(`${file}#${payload.line_index}`, payload.raw);
    }
  }
...
export function listLegacyFiles(store) {
  const files = new Set(LEGACY_TABLE_FILES);
...
export function appendEventsWithEffects(store, events) {
  ...
  for (const [index, event] of events.entries()) {
    ...
    catch (error) { throw new Error(`events[${index}].payload.${error.message}`); }
    for (const effect of effects) affected.add(effect.file);
  }

  return withTransaction(store, () => {
    // Simulate all table effects, including effects earlier in this batch, inside
...
export function renderDirty(store, dataDir, { writeFile = writeFileAtomic } = {}) {
  ...
  for (const { file, dirty: seen } of dirty) {
    try {
