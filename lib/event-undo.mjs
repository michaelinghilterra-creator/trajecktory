...
// that also sets the apply date, a reply that also flips the status). An action can be undone only while it is the
// newest change on its application, so the files go back to exactly what they were; the undo itself is a void event
// (D-10), so the original stays in history.
...
function isMember(leader, e) {
  ...
  if (leader.type === 'status_changed') return e.type === 'application_submitted' && e.id === leader.id + 1;
