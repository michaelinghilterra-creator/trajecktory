...
import { noteEffect } from '../dashboard-web/server/lib/notes.mjs';

export const REPLY_EVENT_TYPE = 'reply_attached';
export const REVIEW_EXCLUDED_TYPE = 'review_item_excluded';
export const LEADER_TYPES = Object.freeze(['status_changed', 'interview_recorded', REPLY_EVENT_TYPE, REVIEW_EXCLUDED_TYPE]);

/** One event describing a reply that was attached to an application and owning its projected note. */
export function buildReplyAttachedEvent({ application_id, msg_id, note_text, note_meta, action, sentiment, status_flip, occurred_on, definitions_version = 'v1' } = {}) {
  const built = noteEffect(application_id, note_text, note_meta);
  return {
    ...
    payload: {
      msg_id,
      note_timestamp: built ? built.item.timestamp : null,
      action, sentiment: sentiment || null, status_flip: status_flip || null,
      legacy_effects: built ? [built.effect] : [],
    },
  };
}
