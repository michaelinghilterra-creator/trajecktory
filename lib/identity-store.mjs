...
function prepared(store, key, sql) {
  let cache = statementCaches.get(store.db);
  if (!cache) {
    cache = new Map();
    statementCaches.set(store.db, cache);
  }
  if (!cache.has(key)) cache.set(key, store.db.prepare(sql));
...
function eventOptions(payload, options = {}) {
  return {
    payload: { ...(options.payload ?? {}), ...payload },
    fields: options.dedupeKey === undefined ? {} : { dedupe_key: options.dedupeKey },
  };
}
...
function resolveId(store, id, { entity, table, mergeTable, fromColumn, intoColumn }) {
  ...
  if (!prepared(store, `resolve:${entity}:exists`, `SELECT id FROM ${table} WHERE id = ?`).get(value)) {
    throw new Error(`${entity} not found: ${value}`);
  }

  const activeMerge = prepared(store, `resolve:${entity}:active-merge`, `
...
function members(store, rootId, {
  ...
  return prepared(store, `members:${mergeTable}`, `
...
function mergeIdentities(store, { fromId, intoId }, ctx, options, callOptions = {}) {
  return withTransaction(store, () => {
    ...
    const [eventId] = insertEvents(store, [contextEvent(
      options.eventType,
      { [options.eventIdField]: intoRoot, ...event.fields },
      event.payload,
      ctx,
    )]);
...
export function mergePeople(store, ids, ctx, options = {}) {
...
function keepSeparate(store, { aId, bId }, ctx, options, callOptions = {}) {
  return withTransaction(store, () => {
...
export function keepPeopleSeparate(store, ids, ctx, options = {}) {
...
function separationExists(store, aRoot, bRoot, options) {
  ...
  return prepared(
    store,
    `separations:${options.table}`,
    `SELECT a_id, b_id FROM ${options.table}`,
  ).all().some(row => (
...
export function findPerson(store, { email, linkedin } = {}) {
  ...
  if (email !== undefined && email !== null && String(email).trim()) {
    ...
    const row = prepared(store, 'person-by-email',
  ...
  if (linkedin !== undefined && linkedin !== null && String(linkedin).trim()) {
    ...
    const row = prepared(store, 'person-by-linkedin',
...
export function findPersonByAlias(store, source, legacyId) {
  const row = prepared(store, 'person-by-alias',
...
export function addPerson(store, { name, emails = [], linkedinUrls = [], aliases = [] }, ctx, options = {}) {
  ...
  return withTransaction(store, () => {
    const conflicts = [];
    const identifierLookup = prepared(store, 'person-identifier-owner',
    ...
    const aliasLookup = prepared(store, 'person-alias-owner',
      'SELECT person_id FROM person_aliases WHERE source = ? AND legacy_id = ?',
    );
    ...
    const [eventId] = insertEvents(store, [contextEvent(
      'person_added',
      { person_id: personId, ...event.fields },
      event.payload,
      ctx,
    )]);
    store.db.prepare(
...
export function addIdentifier(store, personId, { kind, value }, ctx, options = {}) {
  ...
  return withTransaction(store, () => {
    ...
    const existing = prepared(store, 'person-identifier-owner',
      'SELECT person_id FROM person_identifiers WHERE kind = ? AND value = ?',
    ...
    const event = eventOptions(payload, options);
    const [eventId] = insertEvents(store, [contextEvent(
      type,
      { person_id: rootId, ...event.fields },
      event.payload,
      ctx,
    )]);
    store.db.prepare(
...
export function addAlias(store, personId, alias, ctx, options = {}) {
  const normalized = normalizeAlias(alias);
  return withTransaction(store, () => {
    const rootId = resolvePersonId(store, personId);
    const existing = prepared(store, 'person-alias-owner',
      'SELECT person_id FROM person_aliases WHERE source = ? AND legacy_id = ?',
    ).get(normalized.source, normalized.legacyId);
    if (existing && resolvePersonId(store, existing.person_id) === rootId) return false;
    if (existing) {
      throw new IdentityConflictError([{
        kind: normalized.source,
        value: normalized.legacyId,
        existingId: resolvePersonId(store, existing.person_id),
      }]);
    }
    const event = eventOptions({
      alias: { source: normalized.source, legacy_id: normalized.legacyId },
    }, options);
    const [eventId] = insertEvents(store, [contextEvent(
      'person_updated',
      { person_id: rootId, ...event.fields },
    ...
    store.db.prepare(
      'INSERT INTO person_aliases (source, legacy_id, person_id, event_id) VALUES (?, ?, ?, ?)',
    ).run(normalized.source, normalized.legacyId, rootId, eventId);
    return true;
  });
}

export function findCompany(store, name) {
  const row = prepared(store, 'company-by-key', 'SELECT company_id FROM company_keys WHERE key = ?')
    .get(companyKey(name));
...
export function resolveCompany(store, name, ctx) {
  ...
  return withTransaction(store, () => {
    const existing = prepared(store, 'company-by-key', 'SELECT company_id FROM company_keys WHERE key = ?')
      .get(key);
