...
// The key a caller must use for the knownDomains map. Exported because a caller
// that keys it with a DIFFERENT company normalizer loses the lookup silently:
// the domain check simply never fires, and a candidate whose only corroboration
// was an address at that domain is then rejected as uninvented. The repo has
// more than one company normalizer (normCompany strips legal suffixes, this one
// does not), so guessing which is in play is exactly the mistake to prevent.
export function knownDomainKey(company) {
  return normalizeIdentityPart(company);
}
