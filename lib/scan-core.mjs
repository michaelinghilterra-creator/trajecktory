 ...
 *
 * Legacy Texas-specific key names are still read as aliases, so an existing
 * portals.yml keeps working untouched: dfw_core → home_core,
 * hybrid_remote_only → flexible_only, tx_city_coords → region_city_coords. A
 * policy carrying those legacy keys but no explicit `home_region` is treated as
 * a Texas policy, which is what it was.
 */
export function buildLocationFilter(titleFilter) {
  ...
  // home_region wins. Failing that, a policy that still uses the legacy Texas
  // key names IS a Texas policy, so keep its original behavior rather than
  // silently widening a filter the user already tuned. A policy with neither is
  // region-less, and rule 11 stands down (see the docblock).
  const legacyTx = policy.dfw_core || policy.tx_city_coords;
  const regionTokens = (policy.home_region || (legacyTx ? ['texas', ' tx', ', tx'] : []))
    .map(k => k.toLowerCase());
