...
export function buildWeeklyReview({ today, weekCount = 4, applications = [], repliesByApp = new Map(), lastStatusDate = new Map(), applyDates = {}, interviews = [] } = {}) {
  ...
  const allWeeks = recentWeeks(today, weekCount);
  const weeks = allWeeks.map((week, index) => {
    const isCurrentWeek = index === allWeeks.length - 1;
    const unconfirmed_interviews = lines.unconfirmed.filter((i) => inWeek(i.date, week));
    // Scheduled lines are dated AFTER today, and every week here ends on or after
    // today at the latest, so filtering them by week window dropped them silently.
    // Only the current week's window reaches into the future at all, and only as
    // far as its own Saturday: an interview arranged for the week after next
    // vanished from the review entirely, and whether any given one survived
    // depended on which day of the week the review happened to be run. These are
    // forward-looking items rather than a record of a past week, so they all
    // belong to the review being conducted now.
    const scheduled = isCurrentWeek ? lines.scheduled : [];
