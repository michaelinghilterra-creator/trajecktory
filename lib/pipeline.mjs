...
//     repeated "queue clogged" incidents.
...
import { canonicalUrl, sourceUrlFromSnapshot, buildDecidedIndex, urlFromReport } from './identity.mjs';
...
// keeps re-entering the evaluation queue and eventually crowds out genuinely-
...
// unresolved queue entries, because their pipeline row still carried the local:
...
export function sourceUrlOf(url, rootDir) {
  ...
  const sourceUrl = sourceUrlFromSnapshot(url, rootDir);
  return sourceUrl ? canonicalUrl(sourceUrl) : null;
}
...
// ── Discovery Inbox (read-only view) ─────────────────────────────────────────
// Group pipeline.md into exactly what the dashboard needs to SHOW so a
// discovered role is never invisible: still-queued ("- [ ]"), gated-with-reason
// ("- [!]"), and a done count. Before this the dashboard read only evaluated
// rows, so a found-but-pending or gated-dead posting appeared in no view and
// looked "lost" — the core driver of the "we keep losing JDs" complaint.
//
// A row is "<url> | Company | Title", and a gated row appends " <emdash> <reason>"
// after the title. `readable` flags a pending row that already has a local:jds
// snapshot (the eval can read it now) vs a raw ATS URL still awaiting resolve.
export function pipelineInbox(text) {
  const DASH = ' — ';   // the " <emdash> " that precedes a gate reason
  const rows = String(text || '').split('\n').map(parsePipelineRow).filter(Boolean);
  const pending = [], gated = [];
  let done = 0;
  for (const r of rows) {
    const segs = r.rest.split(' | ');
    const company = (segs[1] || '').trim();
    let title = segs.slice(2).join(' | ').trim();
    if (r.state === 'open') {
      pending.push({ url: r.url, company, title, readable: r.url.startsWith('local:') });
    } else if (r.state === 'dead') {
      const i = title.indexOf(DASH);
      const reason = i === -1 ? '' : title.slice(i + DASH.length).trim();
      if (i !== -1) title = title.slice(0, i).trim();
      gated.push({ url: r.url, company, title, reason });
    } else {
      done++;
    }
  }
  return { counts: { pending: pending.length, gated: gated.length, done }, pending, gated };
}
