...
export const MIGRATIONS = Object.freeze([
  ...
  `,
  `
    CREATE TABLE legacy_render_state (
  ...
  `
    CREATE TABLE event_files (
      event_id INTEGER NOT NULL REFERENCES events(id),
      file TEXT NOT NULL,
      UNIQUE (event_id, file)
    );
    CREATE INDEX event_files_file_idx ON event_files(file);

    INSERT OR IGNORE INTO event_files (event_id, file)
    SELECT id, substr(evidence_ref, 1, instr(evidence_ref, '#line') - 1)
    FROM events
    WHERE evidence_ref IS NOT NULL AND instr(evidence_ref, '#line') > 1;

    INSERT OR IGNORE INTO event_files (event_id, file)
    SELECT id, json_extract(payload, '$.file')
    FROM events
    WHERE json_type(payload, '$.file') = 'text'
      AND json_extract(payload, '$.file') <> ''
      AND (json_type(payload, '$.dir') IS NULL OR json_extract(payload, '$.dir') = '');

    INSERT OR IGNORE INTO event_files (event_id, file)
    SELECT id, json_extract(payload, '$.dir') || '/' || json_extract(payload, '$.file')
    FROM events
    WHERE json_type(payload, '$.dir') = 'text'
      AND json_extract(payload, '$.dir') <> ''
      AND json_type(payload, '$.file') = 'text'
      AND json_extract(payload, '$.file') <> '';

    INSERT OR IGNORE INTO event_files (event_id, file)
    SELECT events.id, json_extract(value, '$.file')
    FROM events, json_each(events.payload, '$.legacy_effects')
    WHERE json_type(value, '$.file') = 'text'
      AND json_extract(value, '$.file') <> '';
  `,
]);
...
export function openEventStore(dbPath) {
  ...
  try {
    ...
    for (let index = version; index < MIGRATIONS.length; index++) {
      ...
      try {
        // Another process may have applied this migration while we waited for the lock.
        if (db.prepare('PRAGMA user_version').get().user_version > index) {
          db.exec('COMMIT');
          continue;
        }
...
export function reserveIds(store, name, count = 1, floor = 0) {
  if (typeof name !== 'string' || !/^[a-z][a-z0-9_]*$/.test(name)) {
    throw new Error('name must match /^[a-z][a-z0-9_]*$/');
  }
  ...
  return withTransaction(store, () => {
    const current = store.db.prepare('SELECT value FROM id_counters WHERE name = ?').get(name)?.value ?? 0;
    const start = Math.max(Number(current), floor);
...
export function insertEvents(store, events) {
  ...
  `);
  const ids = [];
  const indexFile = store.db.prepare(
    'INSERT OR IGNORE INTO event_files (event_id, file) VALUES (?, ?)',
  );
  ...
  for (const { event, payload } of validated) {
    ...
    ids.push(eventId);
    const files = new Set();
    const evidenceMatch = event.evidence_ref?.match(/^(.+)#line/);
    if (evidenceMatch) files.add(evidenceMatch[1]);
    if ((!event.payload?.dir || typeof event.payload.dir !== 'string')
      && typeof event.payload?.file === 'string' && event.payload.file) {
      files.add(event.payload.file);
    }
    if (typeof event.payload?.dir === 'string' && event.payload.dir
      && typeof event.payload?.file === 'string' && event.payload.file) {
      files.add(`${event.payload.dir}/${event.payload.file}`);
    }
    for (const effect of Array.isArray(event.payload?.legacy_effects)
      ? event.payload.legacy_effects : []) {
      if (typeof effect?.file === 'string' && effect.file) files.add(effect.file);
    }
    for (const file of files) indexFile.run(eventId, file);
  }
  return ids;
}
