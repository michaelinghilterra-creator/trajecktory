...
// ATS API.
//
// Why this exists: evaluation agents read a posting with a plain
...
export function htmlToText(h) {
  let s = String(h || '')
    ...
    .replace(/<\s*li[^>]*>/gi, '\n- ');
  // Strip tags until the string stops changing. A single pass leaves a partial
  // "<script" behind for nested/crafted input like "<scr<script>ipt>" — the
  // fixed point removes it (CodeQL js/incomplete-multi-character-sanitization).
  let prev;
  do { prev = s; s = s.replace(/<[^>]*>/g, ''); } while (s !== prev);
  // Decode entities. "&amp;" MUST come LAST: decoding it first turns "&amp;lt;"
  // into "<" (a double unescape), so all named/numeric entities are resolved
  // before the ampersand itself (CodeQL js/double-escaping).
  return s
...
export function parsePostingUrl(url) {
  ...
  // Greenhouse, direct board host: (job-boards|boards)[.eu].greenhouse.io/{board}/jobs/{id}
  // The optional region subdomain (.eu.) covers EU-hosted boards (some EU
  // employers were gated "unsupported ATS platform" without it). The US API host
  // boards-api.greenhouse.io serves EU boards too, so fetchJdText needs no
  // change (same approach scan.mjs's detectApi already uses).
  let m = url.match(/(?:job-boards|boards)\.(?:eu\.)?greenhouse\.io\/([^/?#]+)\/jobs\/(\d+)/i);
