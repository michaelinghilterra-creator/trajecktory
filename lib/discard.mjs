...
import { oldLabel } from './passed.mjs';

// An Evaluated entry whose score is BELOW this is auto-discarded (i.e. a 3.5 is
// kept, a 3.4 is discarded). Raised from 3.0 because 3.0 let too many
...
export const AUTO_DISCARD_SCORE = 3.5;
...
// The bottom of the auto-discard band. A Discarded role whose score sits in
// [REQUEUE_FLOOR, AUTO_DISCARD_SCORE), e.g. a noisy 3.4 just under the 3.5 cut,
// is "near threshold": close enough that a re-run could plausibly land it above
// the line, so it stays re-queueable instead of hardening as a permanent reject.
// Below the floor the reject is decisive and not offered for re-queue.
export const REQUEUE_FLOOR = 3.0;

// True when a Discarded row is a near-threshold reject eligible for a one-click
// re-evaluate. Only Discarded rows qualify (a live status is not a reject), and
// only within the band — a clear 1.5 stays discarded. Accepts a numeric score or
// a raw cell.
export function isRequeueableDiscard({ status, score, notes }) {
  if (oldLabel(status, notes) !== 'Discarded') return false;
  const n = typeof score === 'number' ? score : parseScore(score);
  return Number.isFinite(n) && n >= REQUEUE_FLOOR && n < AUTO_DISCARD_SCORE;
}
