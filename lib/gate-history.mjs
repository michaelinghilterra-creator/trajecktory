...
// result is one of: active | expired | uncertain | decided | repost | prefiltered
// (the first three come from the Playwright liveness check; the next two are
...
// prefiltered is spark-prefilter.mjs: a local model scored the posting below the
// configured threshold, so it never reached evaluation. Same question, different
// answer, and the one disposition a user is most likely to want to reverse.)
const VALID_RESULTS = new Set(['active', 'expired', 'uncertain', 'decided', 'repost', 'prefiltered']);
...
// disposition may have changed). Returns { appended: <count actually written> }.
