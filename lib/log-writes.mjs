  ...
  return withLogWrite(dataDir, store => {
    ...
    for (const event of events) {
      for (const effect of event?.payload?.legacy_effects ?? []) {
        if (effectCounts.has(effect)) effectCounts.set(effect, effectCounts.get(effect) + 1);
      }
    }
    if ([...effectCounts.values()].some(count => count !== 1)) {
      throw new Error('buildEvents must attach every table effect to exactly one event');
    }
