...
export function inLogWrite(dataDir) {
  return activeByDir.has(keyFor(dataDir));
}

export function renderPendingResponse(error, context) {
  if (error?.code !== 'RENDER_FAILED') throw error;
  console.warn(`[${context}] ${error.message}`);
  return { render_pending: true, message: error.message };
}

export function logWriteRouteError(res, error) {
  if (error?.code === 'HAND_EDITED') {
    return res.status(409).json({ error: 'hand_edited', files: error.files });
  }
...
export function withLogWrite(dataDir, fn) {
  ...
  let result;
  try {
    result = withProjectionCache(store, () => withTransaction(store, () => {
      const pending = store.db.prepare(
        'SELECT file FROM legacy_render_state WHERE dirty > 0 ORDER BY file',
      ).all().map(row => row.file);
      for (const file of pending) {
        if (fileMatchesLastRender(store, key, file) !== false) continue;
        const projection = renderLegacyFile(store, file);
        const filePath = join(key, file);
        const matchesProjection = projection === null
          ? !existsSync(filePath)
          : existsSync(filePath) && Buffer.compare(readFileSync(filePath), Buffer.from(projection, 'utf8')) === 0;
        if (!matchesProjection) continue;
        const sha256 = projection === null ? ABSENT_FILE_SHA256
          : createHash('sha256').update(Buffer.from(projection, 'utf8')).digest('hex');
        const lastEventId = store.db.prepare('SELECT MAX(id) AS id FROM events').get().id;
        store.db.prepare(`
          UPDATE legacy_render_state
          SET dirty = 0, last_event_id = ?, sha256 = ?, rendered_at = ?
          WHERE file = ?
        `).run(lastEventId, sha256, new Date().toISOString(), file);
      }
      const value = fn(store);
      const dirtyFiles = store.db.prepare(
        'SELECT file FROM legacy_render_state WHERE dirty > 0 ORDER BY file',
      ).all().map(row => row.file);
      const drifted = dirtyFiles.filter(file => {
        const filePath = join(key, file);
        const baselineMatch = fileMatchesLastRender(store, key, file);
        if (baselineMatch === true) return false;
        if (baselineMatch === null) {
          return correspondenceParts(file) !== null && existsSync(filePath);
        }
        const projection = renderLegacyFile(store, file);
      ...
      if (drifted.length) {
        const error = codedError(
          'HAND_EDITED',
          `Files edited outside the app: ${drifted.join(', ')}. Saving is paused and nothing was saved.`,
        );
        error.files = drifted;
        throw error;
      }
...
export function withLogRead(dataDir, fn) {
  const key = keyFor(dataDir);
  const active = activeByDir.get(key);
  if (active) return fn(active);
  return withProjectionCache(openDataStore(key), () => fn(openDataStore(key)));
}

export { localToday } from './local-date.mjs';
