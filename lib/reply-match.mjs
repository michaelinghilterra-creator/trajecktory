...
import { normalizeCompany, roleSignature, sameRoleSignature } from './identity.mjs';
...
// Signatures of every run of up to MAX_ROLE_WORDS words in the text, in scan order.
function windowSignatures(text) {
  ...
  for (let start = 0; start < words.length; start++) {
    ...
    for (let end = start + 1; end <= last; end++) {
      signatures.push(roleSignature(words.slice(start, end).join(' ')));
    }
  }
  return signatures;
}

function roleNamedIn(role, windows) {
  const signature = roleSignature(role);
  return windows.some(window => sameRoleSignature(signature, window));
}
...
export function matchReply(message, applications, { attached = new Map() } = {}) {
  ...
  // The ranking sort calls this on every comparison, so scan the text once per message
  // and answer once per application.
  let windows = null;
  const named = new Map();
  const namesRole = (app) => {
    if (!named.has(app)) named.set(app, roleNamedIn(app.role, windows ??= windowSignatures(text)));
    return named.get(app);
  };
