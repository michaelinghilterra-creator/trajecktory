...
export function changedImportedFiles(store, imported, dataDir) {
  const importedFiles = new Set(Object.keys(imported.texts).filter(correspondenceParts));
  const files = new Set(listLegacyFiles(store));
  for (const dir of LEGACY_CORRESPONDENCE_DIRS) {
    const path = join(dataDir, dir);
    if (!existsSync(path)) continue;
    for (const entry of readdirSync(path, { withFileTypes: true })) {
      const file = `${dir}/${entry.name}`;
      if (entry.isFile() && correspondenceParts(file) && !importedFiles.has(file)) files.add(file);
    }
  }
