...
function acceptedRows(targetTalentText, referralsText) {
  ...
  const withLines = (rows, text, source) => {
    ...
    const used = new Map();
    return rows.map(row => {
      const normalizedRaw = row.raw.replace(/\r$/, '');
      const offset = used.get(normalizedRaw) ?? 0;
      const lineIndex = indexes.get(normalizedRaw)?.[offset];
      used.set(normalizedRaw, offset + 1);
...
export function importPeople(store, {
  ...
  recordFileLayout(store, {
    file: FILES.target_talent,
    text: targetTalentText,
    exists: targetTalentExists,
    rowLineIndexes: parsed.rows
      .filter(row => row.file === FILES.target_talent)
  ...
  recordFileLayout(store, {
    file: FILES.referral,
    text: referralsText,
    exists: referralsExists,
    rowLineIndexes: parsed.rows
      .filter(row => row.file === FILES.referral)
      .map(row => row.line_index),
    definitionsVersion,
    importedOn,
  });
