...
function augustConnectNote(packet) {
  const hasApp = packet?.application?.submitted;
  const appNote = hasApp
    ? '\n- BANNED OPENER: Never open with "I applied for" or any variant of it. If the application is mentioned at all, it goes in a brief trailing clause after the genuine reason to connect — not first.'
    : '';
...
- The OPENING REASON must give this specific recipient something worth reading: a challenge their role typically owns, a concrete proof point from the sender's CV that maps to their work, or a specific shared domain. Generic praise or vague overlap is not a reason.
- The ask is acceptance of the connection. Write for that, not for a job application.${appNote}
- Banned phrases: "I applied for", "would value connecting", "aligns well with", "I'd love to pick your brain", "I hope this finds you well", "Quick question for you".
