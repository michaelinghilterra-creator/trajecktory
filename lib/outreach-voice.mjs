...
function augustConnectNote(packet) {
  return `You are drafting a LinkedIn CONNECTION REQUEST note. A connection request is a request to join someone's professional network. It is NOT an application follow-up, NOT a pitch, and NOT a job inquiry.

PURPOSE: Earn acceptance. The recipient should think "this person is worth knowing" not "this person wants something from me."

IGNORE the APPLICATION section in the fact packet above. A connection request NEVER mentions a job application, an open role, being a candidate, or a job search. That information exists for other message types and must not leak into this one.

HARD RULES:
- Absolute maximum 280 characters. Aim for under 200 (mobile notifications truncate longer notes).
- Open with "Hi ${r(packet).first || 'there'},".
- Give one specific, grounded reason to connect. Reference their work, their team's domain, a shared professional interest, or a concrete observation about their company. The reader should feel you looked at THEIR profile, not that you are mass-sending.
- Peer-to-peer tone. One senior operator to another. Not applicant to gatekeeper.
- No em dashes. No emojis.
- ABSOLUTE BAN: any mention of applying, applications, open roles, job searches, candidacy, or hiring. Also banned: "would value connecting", "aligns well with", "I'd love to pick your brain", "I hope this finds you well", "Quick question for you".
- End with "Thanks, ${s(packet).firstName || 'the sender'}".${voiceBlock(packet)}`;
}
