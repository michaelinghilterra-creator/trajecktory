 ...
 * company that MIGRATED ATS looked brand new. Quennox Ratchet Works (Greenhouse "quennoxold" →
 * Ashby "quennox") and Zorblax Widgetry (Greenhouse "zorblaxwidgetry" → Ashby
 * "zorblax-widgetry") were both re-registered from stale pipeline URLs on
 * one run, each producing a second tracked_companies row pointing at a board
 * that 404s. One surviving row even documented the migration in its
 ...
 *
 *   - punctuation-only drift ("zorblaxwidgetry" vs "zorblax-widgetry"), via the slug key
 *   - a genuinely different slug ("quennoxold" vs "quennox"), via the name key
 *
 * Matching is deliberately conservative. Distinct companies DO share a name —
 * Greenhouse "zorblax" is Zorblax (pet insurance) and Lever "zorblaxpackage" is Zorblax
 ...
 * dropped, everything non-alphanumeric removed. "Zorblax Widgetry", "zorblax-widgetry"
 * and "zorblaxwidgetry" all become "zorblaxwidgetry".
 *
 * Ampersands vanish rather than expanding to "and", so "Zorblax & Sons" →
 * "zorblaxsons" lines up with a "zorblax-sons" slug.
 */
 ...
 *
 * "Quennox (Q-RATCHET)" is one company spelled two ways, so the parenthetical is a
 * real alternate name. "Zorblax (Pet Insurance)" and "Lindy (legacy Ashby slug)"
...
/** Turn an ATS slug into a human display name: "zorblax-widgetry" → "Zorblax Widgetry". */
