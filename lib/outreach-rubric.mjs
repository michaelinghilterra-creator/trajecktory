...
const WRITING_BULLETS = {
  firstSentence: '- The first sentence says plainly why you are writing to this person. If a role you applied for is named in the instructions above, name it.',
  proofPoint: '- Use one proof point, the one that best fits this reader, with any figure copied exactly from the CV excerpt or the verifiable claims. Never invent, round, or combine numbers.',
  companyDetail: '- Use at most one company-specific detail, taken only from the company research, as a short supporting clause. No praise or flattery.',
  ask: '- End with exactly one ask, stated directly in plain words. Do not start the closing sentence with "If". Do not ask for a call, chat, meeting, or an amount of their time. Do not ask to be redirected to an unnamed person.',
  sentenceLength: '- No sentence longer than 25 words. Mix short and longer sentences. Keep paragraphs short.',
...
function howToWrite(profile) {
  const dimensions = new Set(profile.dims.map((dimension) => dimension.id));
  const lines = ['== HOW TO WRITE IT ==', WRITING_BULLETS.firstSentence];
  if (dimensions.has('evidence')) lines.push(WRITING_BULLETS.proofPoint);
  if (dimensions.has('personalization')) lines.push(WRITING_BULLETS.companyDetail);
  if (dimensions.has('ask_strength')) lines.push(WRITING_BULLETS.ask);
  lines.push(
