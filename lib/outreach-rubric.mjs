...
4-5:  One ask, but vague ("would love to connect") or too heavy for a cold contact.
6-7:  Clear and appropriately light, but phrased generically.
8-9:  Specific, easy to say yes to, and matched to how well they know the sender.
10:   Reads as an obvious next step rather than a request.`,
  },
  length_fit: {
    id: 'length_fit',
...
function rubricBody(profile, opts) {
  ...
  if (opts.proofPoints.length) {
    ...
    lines.push('Any metric, headcount, dollar amount, scope figure or date in the message that does not appear in this block or in the CV excerpt above is an INVENTED CLAIM. Score Evidence Grounding at 3 or below and name the invented figure verbatim in the fixes.');
  }
...
export function buildImprovePrompt(surfaceId, opts = {}) {
  ...
  return [
    "Grade the message below against the rubric, then rewrite it so that every fix you name is applied. Preserve every fact, name, metric and the sender's intent. Introduce no new claims.",
    rubricBody(profile, values),
    '== MESSAGE TO IMPROVE ==\n' + message.join('\n\n'),
    generationContract(profile),
  ].join('\n\n');
...
export function parseReviewed(raw, surfaceId) {
  try {
    ...
    const subject = hasSubject ? parsed.subject : undefined;
    const draft = { subject, body: parsed.body, review: null };
