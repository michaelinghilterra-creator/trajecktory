...
export function buildImprovePrompt(surfaceId, opts = {}) {
  ...
  return [
    "Grade the message below against the rubric, then rewrite it so that every fix you name is applied. Preserve every fact, name, metric and the sender's intent. Preserve the opening greeting (e.g. 'Hi [Name],') and closing sign-off (e.g. 'Best,\\nMike') exactly if they appear in the original. Introduce no claim that is not grounded in the supplied sources.",
