...
10:   The recipient would think this person understands what my week actually looks like.`,
  ...
  personalization: {
    ...
    anchors: `Named, checkable references: the requisition, a product, a named team, or a
...
export const RUBRIC_PROFILES = {
  ...
  cover_letter: {
    ...
    dims: [
      { id: 'evidence', weight: 0.26 },
      { id: 'relevance', weight: 0.20 },
      { id: 'personalization', weight: 0.16 },
      { id: 'clarity', weight: 0.16 },
      { id: 'earned_ask', weight: 0.10 },
      { id: 'length_fit', weight: 0.08 },
      { id: 'authenticity', weight: 0.04 },
    ],
    ...
    paragraphs: 3,
    rubric: true,
    note: 'Uses company research for personalization and has a structurally fixed close.',
  },
  short_public: {
    id: 'short_public',
    dims: [
...
const FIRST_TOUCH_SURFACES = new Set([
  'li_followup',
  'ta_dm',
  'ta_email',
  'referral_dm',
...
function isFirstTouchSurface(surfaceId) {
  return surfaceId.startsWith('connect_note_') || FIRST_TOUCH_SURFACES.has(surfaceId);
}
...
function normalizedOptions(opts) {
  ...
  const shortString = (input) => typeof input === 'string' ? input.trim().slice(0, 200) : '';
  return {
    ...
    recipientRole: shortString(value.recipientRole),
    recipientTier: shortString(value.recipientTier),
    appliedRole: shortString(value.appliedRole),
    appliedDate: shortString(value.appliedDate),
    fixes: Array.isArray(value.fixes)
      ? value.fixes.filter((fix) => typeof fix === 'string' && fix.trim()).map((fix) => fix.trim())
      : [],
  };
}

function lengthBlock(profile) {
  const lines = [];
...
function contextBlock(opts) {
  ...
  if (opts.recipientRole || opts.recipientTier || opts.appliedRole || opts.appliedDate) {
    const tierLabels = {
      hm: 'likely hiring manager',
      exec: 'senior executive, not the recruiter',
      peer: 'peer on or near the team',
      ta: 'recruiter or talent partner',
      agency: 'external recruiter',
    };
    const recipientLines = ['== RECIPIENT AND OPENING =='];
    if (opts.recipientRole) recipientLines.push(`Recipient's title: ${opts.recipientRole}`);
    if (opts.recipientTier) recipientLines.push(`Recipient's influence on the hire: ${tierLabels[opts.recipientTier] || opts.recipientTier}`);
    if (opts.appliedRole) {
      recipientLines.push(`Role the sender applied for: ${opts.appliedRole}${opts.appliedDate ? ` (applied ${opts.appliedDate})` : ''}`);
    } else if (opts.appliedDate) {
      recipientLines.push(`Applied: ${opts.appliedDate}`);
    }
    lines.push(recipientLines.join('\n'));
  }

  if (opts.cvExcerpt.trim()) {
    lines.push(['== CV EXCERPT ==', opts.cvExcerpt.trim()].join('\n'));
  }

  if (opts.proofPoints.length) {
    const proofLines = ['== VERIFIABLE CLAIMS =='];
    for (const point of opts.proofPoints) {
      ...
      proofLines.push(`- ${name}: ${heroMetric}`);
    }
    ...
    proofLines.push(`Any metric, headcount, dollar amount, scope figure or date in the message that does not appear in ${allowedSources} is an INVENTED CLAIM. Score Evidence Grounding at 3 or below and name the invented figure verbatim in the fixes.`);
    lines.push(proofLines.join('\n'));
  }

  if (opts.companyResearch) {
    lines.push(['== COMPANY RESEARCH (verified, use for personalization) ==', opts.companyResearch].join('\n'));
  }

  if (opts.superpowers.length) {
    const differentiatorLines = ['== DIFFERENTIATORS =='];
    for (const superpower of opts.superpowers) differentiatorLines.push(`- ${String(superpower)}`);
    lines.push(differentiatorLines.join('\n'));
  }
...
function rubricBody(profile, opts) {
  const lines = [lengthBlock(profile)];
  ...
  const context = contextBlock(opts);
...
const WRITING_BULLETS = {
  role: '- Name the role when known.',
  interest: '- Lead with genuine interest in the company or the work.',
  proofPoints: '- Use one or two proof points with figures copied exactly from the CV excerpt or the verifiable claims. Never invent or round numbers.',
  companyDetail: '- Use at most one company detail and never use market-size, funding, valuation, or hiring-volume statistics.',
  sentenceLength: '- No sentence longer than 40 words.',
  bannedOpeners: '- Banned openers: "I hope this finds you well", "just following up", "just checking in", "touching base", "circling back", "reaching out", "I wanted to reach out", "I am writing to express my interest".',
  bannedWords: '- Banned words: delve, leverage, robust, seamless, spearhead, foster, elevate, unlock, tapestry, pivotal, testament, synergy, game-changer, passionate, results-driven, dynamic, proven track record.',
  dashes: '- No em dashes and no double hyphens.',
};

function howToWrite(profile, firstTouch) {
  const dimensions = new Set(profile.dims.map((dimension) => dimension.id));
  const lines = ['== HOW TO WRITE IT =='];
  if (firstTouch) {
    lines.push(WRITING_BULLETS.role, WRITING_BULLETS.interest);
    if (dimensions.has('evidence')) lines.push(WRITING_BULLETS.proofPoints);
    if (dimensions.has('personalization')) lines.push(WRITING_BULLETS.companyDetail);
  }
  lines.push(
    WRITING_BULLETS.sentenceLength,
    WRITING_BULLETS.bannedOpeners,
    WRITING_BULLETS.bannedWords,
    WRITING_BULLETS.dashes,
  );
  return lines.join('\n');
}

// Writing-only rules for callers that already own the complete fact/context
// block. Deliberately excludes contextBlock() and the output contract so an
// experiment can hold both of those byte-identical while varying only style.
export function buildWritingRules(surfaceId) {
  const profile = getProfile(surfaceId);
  if (!profile || !profile.rubric) return '';
  return [
    lengthBlock(profile),
    howToWrite(profile, isFirstTouchSurface(surfaceId)),
  ].join('\n\n');
}

export function buildWritingGuide(surfaceId, opts = {}) {
  const profile = getProfile(surfaceId);
  if (!profile || !profile.rubric) return '';
...
export function buildImprovePrompt(surfaceId, opts = {}) {
  ...
  const fixes = values.fixes.length
    ? values.fixes.map((fix) => `- ${fix}`)
    : ['- Tighten the weakest sentence without removing any fact.'];
...
function parseReviewedContent(raw, surfaceId, selectContent) {
  try {
    ...
    // Drop dimensions this profile does not carry rather than rejecting the whole
    // review. Profiles deliberately omit dimensions (a connect note drops evidence
    // and earned_ask because 300 characters cannot hold a proof point), and a model
    // that scores one extra out of habit was previously enough to discard an
    // otherwise good review. weightedScore renormalizes over what is present, so a
    // partial set still scores correctly. A review with nothing valid left is still
    // rejected.
    const usableDimensions = Array.isArray(parsed.dimensions)
      ? parsed.dimensions.filter((dimension) => dimension
    ...
    if (!usableDimensions.length || !fixesValid) return draft;

    const profileDims = new Map(profile.dims.map((dimension) => [dimension.id, dimension]));
    const dimensions = usableDimensions.map((dimension) => {
    ...
    const presentIds = new Set(dimensions.map((dimension) => dimension.id));
    const missingDimensions = profile.dims
      .map((dimension) => dimension.id)
      .filter((dimensionId) => !presentIds.has(dimensionId));
    ...
    const review = { score, dimensions, topFixes, weakest };
    if (missingDimensions.length) {
      review.incomplete = true;
      review.missingDimensions = missingDimensions;
    }
...
export function reviewFailureReason(raw, surfaceId) {
  ...
  // Mirror parseReviewed: extras are dropped, so this only reports a failure when
  // nothing usable survives. Reporting on any single unknown id would name a
  // reason the parser no longer rejects for.
  const validIds = new Set(profile.dims.map((dimension) => dimension.id));
  const anyValid = parsed.dimensions.some((dimension) => dimension
    && typeof dimension.id === 'string'
    && validIds.has(dimension.id)
    && Number.isFinite(dimension.score));
  if (!anyValid) return 'bad-dimension-ids';
